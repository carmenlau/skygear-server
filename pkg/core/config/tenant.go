...
func (c *TenantConfiguration) AfterUnmarshal() {
	...
	for i, config := range c.AppConfig.UserVerification.LoginIDKeys {
		// UpdateNilFieldsWithZeroValue by design only handle ptr and map attributes
		// To support UpdateNilFieldsWithZeroValue in list, it should be called
		// seperately for the items
		marshal.UpdateNilFieldsWithZeroValue(&config)
